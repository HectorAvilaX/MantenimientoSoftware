...
class Product:
    ...
    def Vale(self):
        ...
        Label(self.edit_wind, text='Vale').grid(row=1,column=1,columnspan=4)
        Label(self.edit_wind, text= 'Name: ').grid(row=2, column=1)
        new_name = Entry(self.edit_wind)
        new_name.grid(row=2, column=2)

        Label(self.edit_wind, text= 'Tipo: ').grid(row=2, column=3)
        price = Entry(self.edit_wind)
        price.grid(row=2, column=4)

        Label(self.edit_wind, text= 'Obsv: ').grid(row=3, column=1)
        descp = Entry(self.edit_wind)
        descp.grid(row=3, column=2)

        #slect categoria 
        Label(self.edit_wind, text='Producto:').grid(row=3, column=3)
        ...
        #print(lista)
        self.var3=StringVar()
        self.combo = ttk.Combobox(self.edit_wind,textvariable=self.var3)
        self.combo['values']=lista
        self.combo.grid(row=3,column=4)
        self.btn = ttk.Button(self.edit_wind, text='Seleccionar', command=self.selected_item_producto).grid(row=3,column=5)
        self.aux = StringVar()
        print(self.btn)

        Label(self.edit_wind, text='Detalles:').grid(row=4, column=1)
        ...
        #print(lista)
        self.var4=StringVar()
        self.combo2 = ttk.Combobox(self.edit_wind,textvariable=self.var4)
        self.combo2['values']=lista2
        self.combo2.grid(row=4,column=2)
        self.btn2 = ttk.Button(self.edit_wind, text='Seleccionar', command=self.selected_item_vale).grid(row=4,column=3)
        self.aux2 = StringVar()

        fecha = Button(self.edit_wind,text="seleccionar fecha", command=self.grab_date).grid(row=4,column=4)
        self.my_label = Label(self.edit_wind, text='').grid(row=5, column=1)
        self.cal = Calendar(self.edit_wind, selectmode = 'day',year = 2022, month = 5, day = 22)
        self.cal.grid(row=5,column=4)
       #cal.pack(pady = 20)



        Button(self.edit_wind, text='Agregar producto', command=lambda:self.add_product("producto", new_name.get(),price.get(),descp.get(),list(self.aux)[0],list(self.aux2)[0])).grid(row=6, column=1,columnspan=4)
